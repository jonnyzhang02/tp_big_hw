...
LastEditTime: 2023-06-02 14:39:48
...
def generate_tuples(tuple_list):
    ...
    for locations, times, losts, title in tuple_list:
        title = "<"+ title[:-4] + '>'
        title = '暴雨洪涝'
        for location in locations:
            #triplets.append((title, "发生于", location))
            for lost in losts:
                triplets.append((location, title + "造成了损失", lost))
        for time in times:
            for location in locations:
                triplets.append((time, title + "发生于", location))
            for lost in losts:
                triplets.append((time, title + "造成了损失", lost))
